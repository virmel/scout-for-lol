...
import type { AnyMatcher, Matcher, Pattern } from "./Pattern.d.ts";
import type {
  Equal,
  IsUnion,
  MergeUnion,
...
type ReduceFindSelectionUnion<i, ps extends readonly any[], output = never> =
  ps extends readonly [infer head, ...infer tail]
    ? ReduceFindSelectionUnion<i, tail, output | FindSelectionUnion<i, head>>
    : output;
type FindSelectionUnionInArray<i, p, path extends any[] = [], output = never> =
  i extends readonly (infer iItem)[] ? p extends readonly [] ? output
    ...
    :
      | output
      | FindSelectionUnion<iItem, ValueOf<p>, [
        ...path,
        Extract<p, readonly any[]>["length"],
      ]>
    : output;
export type FindSelectionUnion<i, p, path extends any[] = []> = 0 extends 1 & i
  ? never
  : 0 extends 1 & p ? never
  : p extends Primitives ? never
  ...
  : p extends readonly any[] ? FindSelectionUnionInArray<i, p>
  : p extends {} ? i extends {} ? {
...
export type Selections<i, p> = FindSelectionUnion<i, p> extends infer u
  ...
  : i;
export type FindSelected<i, p> = Equal<p, Pattern<i>> extends true ? i
  : Selections<i, p>;
export {};
