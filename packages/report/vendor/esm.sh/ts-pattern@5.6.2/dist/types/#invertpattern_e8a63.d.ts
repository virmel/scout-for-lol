import { DeepExclude } from "./DeepExclude.d.ts";
import {
  All,
  Call,
  Compute,
...
import type { AnyMatcher, Matcher, Override, Pattern } from "./Pattern.d.ts";
type OptionalKeys<p> = ValueOf<
  {
    [k in keyof p]: 0 extends 1 & p[k] ? never
      : p[k] extends Matcher<any, any, infer matcherType>
        ? matcherType extends "optional" ? k : never
      : never;
  }
>;
type ReduceUnion<tuple extends readonly any[], i, output = never> =
  tuple extends readonly [infer p, ...infer tail]
    ? ReduceUnion<tail, i, output | InvertPatternInternal<p, i>>
    : output;
type ReduceIntersection<tuple extends readonly any[], i, output = unknown> =
  tuple extends readonly [infer p, ...infer tail]
...
> = i extends readonly (infer ii)[]
  ...
  : p extends readonly [infer p1, ...infer pRest]
    ...
    : InvertArrayPatternForExclude<pRest, ii[], empty, isReadonly, [
      ...startOutput,
...
type InvertPatternForExcludeInternal<p, i, empty = never> = unknown extends p
  ? i
  : [p] extends [Primitives]
    ? IsLiteral<p> extends true ? p : IsLiteral<i> extends true ? p : empty
  : p extends Matcher<
  ...
  > ? {
      ...
      array: i extends readonly (infer ii)[] ? MaybeAddReadonly<
