...
type Chunk<T extends IterableContainer, N extends number> = T extends
  readonly [] ? []
  : IsNumericLiteral<N> extends true ? LessThan<N, 1> extends true ? never
 ...
 * finite tuples too.
 */
type ChunkFinite<T, N extends number, Result = []> = T extends
  readonly [infer Head, ...infer Rest] ? ChunkFinite<
    Rest,
    N,
    Result extends [
 ...
 * full.
 */
type SuffixChunk<T extends Array<unknown>, Item, N extends number> = T extends
  readonly [] ? [
    ValueOf<
      {
...
type GenericChunk<T extends IterableContainer> = T extends
  readonly [...Array<unknown>, unknown] | readonly [unknown, ...Array<unknown>]
 ...
 * @category Array
 */
declare function chunk<T extends IterableContainer, N extends number>(
  array: T,
  size: N,
): Chunk<T, N>;
/**
 ...
 * @category Array
 */
declare function chunk<N extends number>(
  size: N,
