import {
  ...
  ts,
  TsConfigResolver,
} from "jsr:@ts-morph/common@^0.24.0";
export {
  CompilerOptionsContainer,
  InMemoryFileSystemHost,
  ResolutionHosts,
...
class SourceFileCache {
  ...
  async addOrGetSourceFileFromFilePath(filePath, options) {
    ...
    if (sourceFile == null) {
      const fileText = await this.#fileSystemWrapper.readFileIfExists(
        filePath,
        this.#compilerOptions.getEncoding(),
      );
      if (fileText != null) {
  ...
  addOrGetSourceFileFromFilePathSync(filePath, options) {
    let sourceFile = this.#sourceFilesByFilePath.get(filePath);
    if (sourceFile == null) {
      const fileText = this.#fileSystemWrapper.readFileIfExistsSync(
        filePath,
        this.#compilerOptions.getEncoding(),
      );
      if (fileText != null) {
  ...
  setSourceFile(sourceFile) {
    const standardizedFilePath = this.#fileSystemWrapper
      .getStandardizedAbsolutePath(sourceFile.fileName);
...
async function createProject(options = {}) {
  ...
  if (
    tsConfigResolver != null && options.skipAddingFilesFromTsConfig !== true
  ) {
...
function createProjectSync(options = {}) {
  ...
  if (
    ...
    addSourceFilesForTsConfigResolverSync(
      project,
      tsConfigResolver,
      project.compilerOptions.get(),
    );
...
class Project {
  ...
  constructor(objs, options) {
    ...
    const { languageServiceHost, compilerHost } = createHosts({
      ...
      getProjectVersion: () =>
        this.#sourceFileCache.getProjectVersion().toString(),
  ...
  async addSourceFileAtPath(filePath, options) {
    const sourceFile = await this.addSourceFileAtPathIfExists(
      filePath,
      options,
    );
    if (sourceFile == null) {
  ...
  addSourceFileAtPathIfExists(filePath, options) {
    return this.#sourceFileCache.addOrGetSourceFileFromFilePath(
      this.#fileSystemWrapper.getStandardizedAbsolutePath(filePath),
      {
  ...
  addSourceFileAtPathIfExistsSync(filePath, options) {
    return this.#sourceFileCache.addOrGetSourceFileFromFilePathSync(
      this.#fileSystemWrapper.getStandardizedAbsolutePath(filePath),
      {
        scriptKind: options && options.scriptKind,
      },
  ...
  addSourceFilesFromTsConfig(tsConfigFilePath) {
    ...
    return addSourceFilesForTsConfigResolver(
      this,
      resolver,
      resolver.getCompilerOptions(),
    );
  }
  addSourceFilesFromTsConfigSync(tsConfigFilePath) {
    const resolver = this.#getTsConfigResolver(tsConfigFilePath);
    return addSourceFilesForTsConfigResolverSync(
      this,
      resolver,
      resolver.getCompilerOptions(),
    );
  ...
  #getTsConfigResolver(tsConfigFilePath) {
    const standardizedFilePath = this.#fileSystemWrapper
      .getStandardizedAbsolutePath(tsConfigFilePath);
  ...
  updateSourceFile(filePathOrSourceFile, sourceFileText, options) {
    ...
    function ensureScriptSnapshot(sourceFile) {
      if (sourceFile.scriptSnapshot == null) {
        sourceFile.scriptSnapshot = ts.ScriptSnapshot.fromString(
          sourceFile.text,
        );
      }
    }
  ...
  getLanguageService() {
    return ts.createLanguageService(
      ...
      this.#sourceFileCache.documentRegistry,
    );
  }
  getSourceFileOrThrow(fileNameOrSearchFunction) {
    ...
    if (typeof fileNameOrSearchFunction === "string") {
      const fileNameOrPath = FileUtils.standardizeSlashes(
        fileNameOrSearchFunction,
      );
      if (
        FileUtils.pathIsAbsolute(fileNameOrPath) ||
        fileNameOrPath.indexOf("/") >= 0
      ) {
        const errorFileNameOrPath = this.#fileSystemWrapper
          .getStandardizedAbsolutePath(fileNameOrPath);
        throw new errors.InvalidOperationError(
  ...
  #getSourceFileInternal(fileNameOrSearchFunction) {
    ...
    if (isStandardizedFilePath(filePathOrSearchFunction)) {
      return this.#sourceFileCache.getSourceFileFromCacheFromFilePath(
        filePathOrSearchFunction,
      );
    }
    ...
    function selectSmallestDirPathResult(results) {
      ...
      for (const sourceFile of results) {
        if (
          result == null ||
          FileUtils.getDirPath(sourceFile.fileName).length <
            FileUtils.getDirPath(result.fileName).length
        ) {
          result = sourceFile;
        }
      }
...
function addSourceFilesForTsConfigResolverSync(
  project,
  tsConfigResolver,
  compilerOptions,
) {
  return tsConfigResolver.getPaths(compilerOptions).filePaths.map((p) =>
    project.addSourceFileAtPathSync(p)
  );
}

export { createProject, createProjectSync, Project };
