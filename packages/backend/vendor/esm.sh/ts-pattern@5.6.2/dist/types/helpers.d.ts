...
export type UnionToIntersection<union> =
  (union extends any ? (k: union) => void : never) extends
    (k: infer intersection) => void ? intersection : never;
...
export type UnionToTuple<union, output extends any[] = []> =
  UnionToIntersection<union extends any ? (t: union) => union : never> extends
    ...
    : output;
export type Flatten<xs extends readonly any[], output extends any[] = []> =
...
export type Iterator<n extends number, it extends any[] = []> =
...
> = Length<it> extends 0 ? output
  ...
  : output;
export type Drop<xs extends readonly any[], n extends any[]> = Length<n> extends
  0 ? xs : xs extends readonly [any, ...infer tail] ? Drop<tail, Prev<n>> : [];
...
> = Length<n> extends 0
  ? tail extends readonly [any, ...infer tail] ? [...inits, value, ...tail]
  : inits
...
export type IsPlainObject<o, excludeUnion = BuiltInObjects> = o extends object
  ? o extends string | excludeUnion ? false : true
  : false;
export type Compute<a extends any> = a extends BuiltInObjects ? a : {
  [k in keyof a]: a[k];
};
export type IntersectObjects<a> = (a extends any ? keyof a : never) extends
  infer allKeys ? {
...
export type IsLiteral<a> = [a] extends [null | undefined] ? true
 ...
 */
export type GuardValue<fn> = fn extends (value: any) => value is infer b ? b
  : fn extends (value: infer a) => unknown ? a
  : never;
export type GuardFunction<input, narrowed> =
  | ((value: input) => value is Extract<narrowed, input>)
  | ((value: input) => boolean);
export type Some<bools extends boolean[]> = true extends bools[number] ? true
  : false;
export type All<bools extends boolean[]> = bools[number] extends true ? true
  : false;
...
export type IsTuple<a> = a extends
  readonly [] | readonly [any, ...any] | readonly [...any, any] ? true : false;
export type IsStrictArray<a extends readonly any[]> = Not<IsTuple<a>>;
export type IsReadonlyArray<a> = a extends readonly any[]
  ? a extends any[] ? false : true
  : false;
export type MaybeAddReadonly<a, shouldAdd extends boolean> = shouldAdd extends
  true ? Readonly<a> : a;
export type MapKey<T> = T extends Map<infer K, any> ? K : never;
export type MapValue<T> = T extends Map<any, infer V> ? V : never;
export type SetValue<T> = T extends Set<infer V> ? V : never;
export type ReadonlyArrayValue<T> = T extends ReadonlyArray<infer V> ? V
  : never;
export type ExtractPlainObject<T> = T extends any
  ? IsPlainObject<T> extends true ? T : never
  : never;
export type GetKey<O, K> = O extends any ? K extends keyof O ? O[K] : never
  : never;
export interface Fn {
  input: unknown;
  output: unknown;
