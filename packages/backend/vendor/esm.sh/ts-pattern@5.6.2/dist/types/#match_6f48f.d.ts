import type * as symbols from "../internals/symbols.d.ts";
import type { MatchedValue, Pattern } from "./Pattern.d.ts";
import type {
  InvertPattern,
  InvertPatternForExclude,
} from "./InvertPattern.d.ts";
import type { DeepExclude } from "./DeepExclude.d.ts";
import type { GuardValue, IsNever, Union } from "./helpers.d.ts";
import type { FindSelected } from "./FindSelected.d.ts";
...
> = {
  ...
  >(p1: p1, p2: p2, handler: (value: value) => PickReturnValue<o, c>): [
  ...
  >(
    ...
    infer excludedRest,
      ...
      Exclude<
        i,
        excluded1 | excluded2 | excluded3 | Extract<excludedRest, any[]>[number]
      >,
      o,
      [
        ...handledCases,
  ...
  returnType: [inferredOutput] extends [never]
    ...
    : TSPatternError<
      "calling `.returnType<T>()` is only allowed directly after `match(...)`."
    >;
};
/**
 ...
 */
type DeepExcludeAll<a, tupleList extends any[]> = [a] extends [never] ? never
