...
> = IsBoundedKey<KeysOfUnion<T>> extends true ? TypeIfBoundedRecord
...
type IsBoundedKey<T> = T extends unknown
  ? IsStringLiteral<T> extends true ? IsBoundedString<T>
  : IsNumericLiteral<T> extends true ? true
  : IsSymbolLiteral<T>
  : never;
/**
...
type IsBoundedString<T> = T extends string
  ? Split<T, "">[number] extends infer U ? [
      `${number}`,
    ] extends [U] ? false : [string] extends [U] ? false : true
  : false
  : false;
