...
type Drop<T extends IterableContainer, N extends number> = IsNegative<N> extends
  true ? T
  : IsInteger<N> extends false ? Array<T[number]>
 ...
 */
type DropUpTo<T, N, Dropped extends ReadonlyArray<unknown> = []> =
  Dropped["length"] extends N ? T : T extends [unknown, ...infer Rest] // Take the current value, and then recurse with the array where it is
 ...
 * @category Array
 */
declare function drop<T extends IterableContainer, N extends number>(
  array: T,
  n: N,
): Drop<T, N>;
/**
 ...
 * @category Array
 */
declare function drop<N extends number>(
  n: N,
