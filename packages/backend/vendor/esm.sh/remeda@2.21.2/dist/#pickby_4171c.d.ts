...
type ExactProps<T, S> = {
  -readonly [
    P in keyof T as EnumerableKey<IsExactProp<T, P, S> extends true ? P : never>
  ]: Extract<Required<T>[P], S>;
};
type PartialProps<T, S> = {
  -readonly [
    P in keyof T as EnumerableKey<
      IsPartialProp<T, P, S> extends true ? P : never
    >
  ]?: IfNever<Extract<T[P], S>, S extends T[P] ? S : never, Extract<T[P], S>>;
};
type IsExactProp<T, P extends keyof T, S> = T[P] extends Extract<T[P], S> ? true
  : false;
type IsPartialProp<T, P extends keyof T, S> = IsExactProp<T, P, S> extends true
  ? false
  : IfNever<Extract<T[P], S>, S extends T[P] ? true : false, true>;
/**
