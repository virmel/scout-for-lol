...
> =
  ...
  & {
    [Key in keyof Destination as Key extends keyof Source ? never : Key]:
      Destination[Key];
  }
  // Case in rule 2: The source contains the key but the destination doesn't.
  & {
    [Key in keyof Source as Key extends keyof Destination ? never : Key]:
      Source[Key];
  }
...
type PickRestTypeHelper<Tail extends UnknownArrayOrTuple, Type> = Tail extends
  [] ? Type : PickRestType<Tail>;
...
```
*/
type PickRestType<Type extends UnknownArrayOrTuple> = number extends
  Type["length"] ? PickRestTypeHelper<ArrayTail<Type>, Type>
  : [];
...
type TypeNumberOrType<Type extends UnknownArrayOrTuple> = Type[number] extends
  never ? Type : Type[number];
...
type DoMergeDeepTupleAndTupleRecursive<
  ...
  SourceRestType,
  Options extends MergeDeepInternalOptions,
> = Destination extends [] ? Source extends [] ? []
...
type DoMergeArrayOrTuple<
  ...
  Source extends UnknownArrayOrTuple,
  Options extends MergeDeepInternalOptions,
> = ShouldSpread<Options> extends true ? Array<
...
> = Destination[number] extends UnknownArrayOrTuple
  ? Source[number] extends UnknownArrayOrTuple ? Array<
  ...
  : Destination[number] extends UnknownRecord
    ? Source[number] extends UnknownRecord ? Array<
...
> = SimplifyDeepExcludeArray<
  [undefined] extends [Destination | Source] ? DefaultType
    : Destination extends UnknownRecord
      ...
      : DefaultType
    : Destination extends UnknownArrayOrTuple
      ? Source extends UnknownArrayOrTuple ? MergeDeepArrayOrTuple<
          Destination,
          Source,
