...
export type Paths<T, Options extends PathsOptions = {}> = _Paths<T, {
  ...
  leavesOnly: Options["leavesOnly"] extends boolean ? Options["leavesOnly"]
    : DefaultPathsOptions["leavesOnly"];
  // Set default depth to number
  depth: Options["depth"] extends number ? Options["depth"]
    : DefaultPathsOptions["depth"];
}>;
          ...
          ? (
            Options["bracketNotation"] extends true
              ...
              // If `Key` is a number, return `Key | `${Key}``, because both `array[0]` and `array['0']` work.
                ? (Key | ToString<Key>)
              : never
          ) extends infer TranformedKey extends string | number // 1. If style is 'a[0].b' and 'Key' is a numberlike value like 3 or '3', transform 'Key' to `[${Key}]`, else to `${Key}` | Key
            // 2. If style is 'a.0.b', transform 'Key' to `${Key}` | Key
            ?
              | ((Options["leavesOnly"] extends true
                ? MaxDepth extends 0 ? TranformedKey
                : T[Key] extends
                  | EmptyObject
                  | readonly []
                  | NonRecursiveType
                  | ReadonlyMap<unknown, unknown>
                  | ReadonlySet<unknown> ? TranformedKey
                : never
                : TranformedKey) extends infer _TransformedKey
                ...
                ? 0 extends Options["depth"] ? _TransformedKey
                : never
                : never)
              | (
                ...
                GreaterThan<MaxDepth, 0> extends true // Limit the depth to prevent infinite recursion
                  ...
                  }> extends infer SubPath ? SubPath extends string | number ?
                        | (
                          Options["bracketNotation"] extends true
