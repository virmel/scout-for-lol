...
interface VariableAccessors<AlternateType = unknown> {
   ...
   * Converts a number to an integer and verifies it's in port ranges 0-65535
   */
  asPortNumber: () => AlternateType extends undefined ? undefined | number
    : number;
   ...
   * Performs the same task as asFloat(), but also verifies that the number is positive (greater than or equal to zero).
   */
  asFloatPositive: () => AlternateType extends undefined ? undefined | number
    : number;

  /**
   * Performs the same task as asFloat(), but also verifies that the number is negative (less than or equal to zero).
   */
  asFloatNegative: () => AlternateType extends undefined ? undefined | number
    : number;
   ...
   * Performs the same task as asInt(), but also verifies that the number is positive (greater than or equal to zero).
   */
  asIntPositive: () => AlternateType extends undefined ? undefined | number
    : number;

  /**
   * Performs the same task as asInt(), but also verifies that the number is negative (less than or equal to zero).
   */
  asIntNegative: () => AlternateType extends undefined ? undefined | number
    : number;
   ...
   * Return the variable value as an Email. Throws an exception if value is not an Email.
   */
  asEmailString: () => AlternateType extends undefined ? undefined | string
    : string;

  /**
   * Attempt to parse the variable to a JSON Object or Array. Throws an exception if parsing fails.
   */
  asJson: () => AlternateType extends undefined
    ? undefined | Object | Array<any>
    : Object | Array<any>;

  /**
   * The same as asJson but checks that the data is a JSON Array, e.g [1,2].
   */
  asJsonArray: () => AlternateType extends undefined ? undefined | Array<any>
    : Array<any>;

  /**
   * The same as asJson but checks that the data is a JSON Object, e.g {a: 1}.
   */
  asJsonObject: () => AlternateType extends undefined ? undefined | Object
    : Object;
  ...
  asArray: (
    delimiter?: string,
  ) => AlternateType extends undefined ? undefined | Array<string>
    : Array<string>;
  ...
  asSet: (
    delimiter?: string,
  ) => AlternateType extends undefined ? undefined | Set<string> : Set<string>;
   ...
   * The var must be set to either "true" or "false" (upper or lowercase) to succeed.
   */
  asBoolStrict: () => AlternateType extends undefined ? undefined | boolean
    : boolean;
   ...
   * valid URL strings, instead of error prone string concatenation.
   */
  asUrlString: () => AlternateType extends undefined ? undefined | string
    : string;
  ...
  asEnum: <T extends string>(
    validValues: readonly T[] | T[],
  ) => AlternateType extends undefined ? undefined | T : T;
  ...
  asRegExp: (
    flags?: string,
  ) => AlternateType extends undefined ? undefined | RegExp : RegExp;
}

interface IPresentVariable<Exs extends Extensions = {}>
  extends VariableAccessors {
  /**
  ...
  ) => IPresentVariable<Exs> & ExtenderType<Exs>;
...
interface IOptionalVariable<Exs extends Extensions = {}>
  extends VariableAccessors<undefined> {
  /**
  ...
  example: (
    value: string,
  ) => IOptionalVariable<Exs> & ExtenderTypeOptional<Exs>;
...
type RestParams<F extends (...args: any[]) => any> = F extends
  (value: string, ...args: infer P) => any ? P : any[];
type ExtenderType<T extends Extensions> = {
  [P in keyof T]: (...args: RestParams<T[P]>) => ReturnType<T[P]>;
};
type ExtenderTypeOptional<T extends Extensions> = {
  [P in keyof T]: (...args: RestParams<T[P]>) => ReturnType<T[P]> | undefined;
};
